...
This document provides detailed design system documentation for the Side Hustle project.
...
The gallery system provides a responsive grid layout for browsing art and photography with filtering, sorting, and product detail pages.

**Gallery CSS Variables:**
```css
/* Grid Settings */
--gallery-grid-gap: var(--space-6)           /* 24px gap between cards */
--gallery-card-radius: var(--radius-md)       /* Card border radius */
--gallery-thumbnail-aspect: var(--hero-frame-aspect)  /* 3:4 aspect ratio */

/* Filter Bar */
--gallery-filter-bg: var(--color-bg-secondary)
--gallery-filter-padding: var(--space-4)

/* Dark Mode Overrides */
--gallery-card-bg: var(--color-neutral-800)
--gallery-card-border: var(--color-neutral-700)
```
...
- Consistent gap using `--gallery-grid-gap`
...
**Gallery Filter Bar (`.gallery-filter-bar`):**
```css
...
**Filter Controls:**
- Type filter: Art/Photography selection
- Sub-category filter: Dynamic options based on type
- Sort select: Newest, Oldest, Title A-Z, Title Z-A
- Clear button: Resets all filters to default

**Product Detail (`.product-detail`):**
```css
.product-detail: Main container for detail page
.product-detail-header: Back link and title area
.product-image-container: Large image display
.product-info: Title, description, metadata
```

**Print Options Table (`.print-options-table`):**
- Displays available print sizes and pricing
- Responsive table with horizontal scroll on mobile
- Styled header and alternating row colors

**Frame Options Grid (`.frame-options-grid`):**
- Visual preview of framing options
- 4-column grid with frame preview images
- Labels below each option

**Usage Example:**
```html
<!-- Gallery Grid -->
<div class="gallery-filter-bar" role="toolbar" aria-label="Gallery filters">
...
<div class="gallery-container">
    <div id="gallery-grid" class="gallery-grid" role="list" aria-label="Gallery items">
...
**JavaScript Modules:**
- `gallery-data.js`: Data layer with categories, items, print/frame options
- `gallery-grid.js`: Grid rendering, filtering, sorting, sessionStorage persistence
- `gallery-detail.js`: Product detail page rendering

**Accessibility:**
- ARIA roles: `toolbar`, `list` for semantic structure
- ARIA labels on all interactive elements
- Keyboard navigation with visible focus states
- Image alt text for all thumbnails

**State Persistence:**
Filter and sort selections persist across page navigation using sessionStorage:
- `gallery-filter-type`: Selected type filter
- `gallery-filter-subcategory`: Selected sub-category
- `gallery-sort`: Current sort order
